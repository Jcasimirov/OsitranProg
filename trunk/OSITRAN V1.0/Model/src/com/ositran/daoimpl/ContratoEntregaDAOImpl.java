...
public class ContratoEntregaDAOImpl implements ContratoEntregaDAO {
    ...
    public List<ContratoEntrega> query() throws SQLException {
        ...
        List list=session.createQuery("select o from ContratoEntrega ca o where ca.cadEstado != 0").list();
    ...
    public List<ContratoEntrega> getEntregasContrato(Integer conId) throws SQLException{
        ...
        session.beginTransaction();
        Query query; 
        query = session.createQuery("select o from ContratoEntrega ca where ca.cenEstado <> 0 and ca.conId = :busqueda1 order by CEN_ID DESC");
