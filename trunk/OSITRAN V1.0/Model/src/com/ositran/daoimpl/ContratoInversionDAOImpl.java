...
public class ContratoInversionDAOImpl implements ContratoInversionDAO {
    ...
    public String updateInversionxInfraestructuras(int contratoId, int csiId, int tinId, int infId, String usuario,
        ...
        try {
            ...
            Query query =
                session.createSQLQuery(" UPDATE T_CONTRATO_INVERSION " + " SET   INV_ESTADO = 0," +
                                       ...
                                       "       INV_TERMINAL       = :terminal " + " WHERE CON_ID            = " +
                                       contratoId + " " + "   AND CSI_ID            = " + csiId + " " +
                                       "   AND TIN_ID            = " + tinId + " " + "   AND INF_ID            = " +
                                       infId);
    ...
    public boolean validaNoExisteNombreInversion(int contratoId, int csiId, int tinId, int infId,
        ...
        try {
            tx = session.beginTransaction();
            Query query =
                session.createQuery(" SELECT count(M.invId) FROM ContratoInversion M " + " WHERE M.conId = " +
                                    contratoId + " " + "   AND M.csiId = " + csiId + " " + "   AND M.tinId = " + tinId +
                                    " " + "   AND M.infId = " + infId + " " + " AND upper(trim(M.invDescripcion)) = '" +
                                    nombreInversion + "'");
